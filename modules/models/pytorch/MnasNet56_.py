...
class MnasNet56_(nn.Module):
    def __init__(self, input_size=224, width_mult=1.):
        super(MnasNet56_, self).__init__()
        ...
        self.upsampling = nn.Sequential(
            # pw
            nn.Conv2d(self.last_channel, self.last_channel, 1, 1, 0, bias=False),
            nn.BatchNorm2d(self.last_channel),
            nn.ReLU(inplace=True),

            nn.ConvTranspose2d(self.last_channel, self.last_channel, 4, 2, 1),
            nn.BatchNorm2d(self.last_channel),
            nn.ReLU(inplace=True),

            # pw-linear
            nn.Conv2d(self.last_channel, 192, 1, 1, 0, bias=False),
            nn.BatchNorm2d(192),
            ...
            # pw-linear
            nn.Conv2d(192, 80, 1, 1, 0, bias=False),
            nn.BatchNorm2d(80),
        )

        self.heatmap = nn.ConvTranspose2d(80, self.Nj, 4, 2, 1)
        self.offset = nn.ConvTranspose2d(80, self.Nj*2, 4, 2, 1)
